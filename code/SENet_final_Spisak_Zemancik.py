...
acc, prec, rec, f1 = evaluate_model(m1, loader_test, full_ds.classes)
print(f"TL-only →  Acc: {acc:.4f}  Prec: {prec:.4f}  Rec: {rec:.4f}  F1: {f1:.4f}")
results['TL-only'] = acc
...
acc, prec, rec, f1 = evaluate_model(m2, loader_test, full_ds.classes)
print(f"TL-DA →  Acc: {acc:.4f}  Prec: {prec:.4f}  Rec: {rec:.4f}  F1: {f1:.4f}")
results['TL-DA'] = acc
...
acc, prec, rec, f1 = evaluate_model(m3, loader_test, full_ds.classes)
print(f"TL+DA+SENet →  Acc: {acc:.4f}  Prec: {prec:.4f}  Rec: {rec:.4f}  F1: {f1:.4f}")
results['TL+DA+SENet'] = acc
